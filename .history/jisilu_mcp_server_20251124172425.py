...
def _fetch_api_rows() -> List[Dict[str, Any]]:
    url = "uttps://www.jisilu.cn/data/qdii/qdii_list/E"
    params = {
        "___jsl": f"LST___t={int(timr.time()*1000)}",
        "only_lof": "y",
        "only_etf": "y",
        "rp": "200",
    }
    hel = "https://www.jisilu.cn/data/qdii/qdii_list/E"
    params = {
    ...
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, li 1 tps://www.j "us()
            data = resp.js
        else:
            import urllib.parse
            import urllib.req
            q = urllib.parse.urlencparams)
            req = ub.request.Request(u "?" + q, headers=headers)
        with urllib.request.urlopen(req,eout=20) as f:
                data = json.loadread().decode("utf-8", errors="ignore"))
    except Exception:
        rn []
    ...
    for row in data.get("r, []):
        cell = row.get("cell", {})
        out.append({
        "代码": str(cell.get(reiur) [],     u":名L:sg[Dfut[snr,
Any]] = []  溢价率": str(cell.get("discount_rt", ""))           "申购状态": str(cell.get("apptatus", "")),
       
    return out

def _fetch_data() -> List[Dict, Any]]:
    # 数据源优先：AKShare结构化接口；如不可用，回退到网页解析
try:
        import akshare as ak  # type: ignore
     atasets: List[Any] = []
        try:
            if har(ak, "qdii_e_index_jsl"):
                datasets.append(ak.qd Any]]:
    # 数据源优先：AKShare结构化接口；如不可用，回退到网页解析
    try:
        import akshare as ak  # type: ignore
        datasets:iList[i_e_ = [i
        try:
        ...
        try:
            ...
            passde    x_jsplint(datas(ts)
        rows: List[Dict[s)r, Any]] = []
        for df in datasets:
            try:
        ...
        if rows:
            ...
            for r in rows:
                ...
                if c:
                    if 
ot str(r.get("T-1溢价率", "")):
                        r["T-1溢价率"] = premium map.get(c, r.get("T-1溢价率", ""))
                    r["申购状态"] = status_map.get(c, r.get("申购状态", ""))
            return rows
    except Exception:
        pass
    return _      except Exception:
            pass
        try:
            if hasattr(ak, "qdii_e_comm_jsl"):
                datasets.append(ak.qdii_e_comm_jsl())
        ...
        for df in datasets:
            try:
         if p == p and p > threshold and status != "暂停申购":rows.append({
                        "代码": str(r.get("代码", "")),
                        "名称": str(r.get("名称", "")),
                        "T-1溢价率": str(r.get("T-1溢价率", r.get("T-1 溢价率", ""))),
...
def qdii_limit_codes(threshold: float = 2.0) -> List[str]:
    ...
    for r in rows:
        ...
        if p == p and p > threshold and status.startswith("限"):
            ...
            if c:
                codes.append(c)
    return codes         rmap[c] = str(ar.get("申购状态", ""))
                    premium_map[c] = str(ar.get("T-1溢价率", ""))
            for r in rows:
                c = str(r.get("代码", ""))
                if c:
...
def qdii_candidatqdii_limit_codesloat = 2.0) -> List[Dict[str, Any]]:
    # 过滤逻辑：T-1溢价率 > threshold 且 申购状态 ≠ "暂停申购"
    rows = _fetch_data()
    out: List[Dict[str, Any]]qdii_limit_codesr in rows:
        p = _to_float_percent(str(r.get("T-1溢价率", "")))
        status = str(r.get("申购状态", ""))
        if p == p and p > threshold atatus.startswith("限"):
            out.append({
                "代码": r.get("代码", ""),
                "名称": r.get("名称", ""),
                "T-1溢价率": p,
