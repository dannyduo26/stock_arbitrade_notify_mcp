...
def _fetch_data() -> List[Dict[str, Any]]:
    try:
        ...
        if rows:
            ...
            for code in {r["代码"] for r in rows if r.get("代码")}:
                m = re.search(rf"{re.escape(code)}.*?申购状态\s*([\u4e00-\u9fa5A-Za-z0-9%]+)", plain)
                if m:
                    status_map[code] = m.group(1)
            for r in rows:
                r["申购状态"] = status_map.get(r.get("代码", ""), "")
            return rows
    except Exception:
        pass
    html = _fetch_html(URL)
...
def qdii_candidates(threshold: float = 2.0) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        if p == p and p > threshold and status and status != "暂停申购":
            out.append({
                "代码": r.get("代码", ""),
                "名称": r.get("名称", ""),
                "T-1溢价率": p,
